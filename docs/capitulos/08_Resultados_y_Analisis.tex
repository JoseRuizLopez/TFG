...
En cambio, el algoritmo aleatorio presenta una mayor dispersión hacia valores bajos y aunque presente una mayor sensibilidad a la aleatoriedad de las selecciones,
puede llegar a tener mejores valores mínimos, como se evidencia en su menor valor mínimo (0.785 frente a 0.769 en búsqueda local),
pero siendo el de la busqueda local un valor atípico.
...
\begin{figure}[htp]
    ...
    \caption{Diagrama de barras de \textit{accuracy} según el porcentaje inicial de datos.}
...
\begin{table}[htp]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{P{2.5cm} P{2cm} P{2.5cm} P{2.5cm} P{2.5cm} P{2.5cm} P{2cm}}
            \toprule
            \textbf{Algoritmo} & \textbf{Accuracy (Avg)} & \textbf{Precision (Avg)} & \textbf{Recall (Avg)} & \textbf{F1-score (Avg)} & \textbf{Evaluaciones} & \textbf{Porc. Final} \\
            \midrule
